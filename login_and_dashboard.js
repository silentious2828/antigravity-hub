...
(async () => {
  ...
  console.log('Login submitted, checking dashboard...');
  // Step 2: Navigate to dashboard endpoint
  await page.goto('http://localhost:20128/dashboard/endpoint', { waitUntil: 'networkidle' });
  // Wait for page to load
  await page.waitForLoadState('networkidle');
  // Take a screenshot to verify
  await page.screenshot({ path: 'dashboard.png' });
  console.log('Dashboard screenshot saved as dashboard.png');
  // Optionally, you can assert some element exists on the dashboard
  const dashboardTitle = page.locator('h1');
  await dashboardTitle.waitFor({ state: 'attached' });
